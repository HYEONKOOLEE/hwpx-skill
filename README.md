...
> **버전** v1.3.0 · **최종 수정** 2026-09-04
>
> **이 저장소 하나로 완결됩니다. 함께 설치해야 하는 다른 스킬은 없습니다.**
>
> **v1.3 변경점**: **표 구조 무결성 검사(R4)** 를 추가했습니다. 서식 문서의 표에서 빈 행을
> 지울 때 세로 병합(`rowSpan`) 범위를 함께 줄이지 않으면 한글이 *"파일을 읽거나 저장하는데
> 오류가 있습니다"* 로 파일 자체를 거부합니다. 문제는 기존 검증 도구가 이를 **전부 통과**
> 시켰다는 점이라, `verify_hwpx.py`에 검사 2종(표 병합 정합성 · 단락 id 중복)을 넣고
> `SKILL.md`에 절대 규칙 R4와 트러블슈팅 절을 신설했습니다.
...
1. `dist/hwpx-v1.3.0.skill` 파일을 클로드 대화창에 올립니다
2. 나타나는 **저장** 버튼을 누릅니다
...
작성일: 2026-09-04 | 버전: v1.3.0 | 작성: 프랭크 × 에이미 협업
